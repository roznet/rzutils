...
// Temporary use of approximate-equality in swift until formally included
// from: https://github.com/apple/swift-evolution/blob/main/proposals/0259-approximately-equal.md
...
extension Measurement<Dimension> {
    ...
    public func isAlmostEqual(
      ...
      tolerance: Double = Double.ulpOfOne.squareRoot()
        ...
        return other.converted(to: self.unit).value.isAlmostEqual(to: self.value, tolerance: tolerance)
    }
    
    @inlinable
    public func isAlmostZero(
      absoluteTolerance tolerance: Double = Double.ulpOfOne.squareRoot()
    ) -> Bool {
        return self.value.isAlmostZero(absoluteTolerance: tolerance)
    }
}
